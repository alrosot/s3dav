...
public class FTPConnection implements Runnable {
    ...
    public FTPConnection(FTPServer p, Socket i, FTPObject startingObject,
            FTPObject root, S3Log log) {
        _log = log;
        _isLoggedIn = false;
        parent = p;
    ...
    public void rebuildFlows(Socket i) {
        try {
            outflow = new PrintWriter(new OutputStreamWriter(incoming
                    .getOutputStream()), true);
            inflow = new BufferedReader(new InputStreamReader(incoming
                    .getInputStream()));
        } catch (Exception e) {
    ...
    boolean isValidUser(String userName) {
        return parent.isValidUser(userName);
    }

    boolean isValidPassword(String password) {
        _isLoggedIn = parent.isValidPassword(password);
        return _isLoggedIn;
    }
    ...
    public void pwd() {
        output("257 \"" + getCurrentDirectory() + "\" is current directory");
    }
    ...
    public void storI(String commandArgs, boolean append) {
        ...
        File saveFile = new File(getPhysicalDirectory()
    ...
    private void retrA(String commandArgs) {
        ...
        try {
            if (getResumePosition() > 0)
                dataStream.skip(getResumePosition());
